...
The TMC471 communicates with our MCU using the SPI protocol. SPI fits our system well, as it allows our master (MCU) to have several slaves (motors) on fewer lines.

The TMC471 needs extensive input to ensure a precise output. It takes in hall sensor data directly from its opposing BLDC motor, and reads current sensors to regulate torque. Each of these signals are 
...
/* Agree? */
We have disconnected the BRAKE pin. The pin is not intended for braking the motors, but for dissipating excess energy provided for the motors voltage feedback. Outside of scope for now. 

REF_R. REF_H, REF_L are pins intended to be driven HIGH if we can tell if the motors are approaching the physical limits of motion. This is overkill for our project, as there are no real scenarios wherein we would deem this necessary. Our motors only control a flywheel clockwise or counter-clockwise. We wont ever be in a position where this is no more degrees we can spin in a given direction physically. 

The ENI (Enable Input)/ENO (Enable Output) pins are two separate pins that co-operate. Enable Input essentially acts like a kill switch for the motor control in general. If we 
reach a bad state, we can drive ENI low, and the motors will chill out. ENO is an output pin, which just reflects the state of ENI, and is intended for more self-reliant embedded systems. Again, we control everything with SPI. Therefore we will have the ENI pin as a killswitch, while having ENO disconnected.

The STEP and DIR pins are not needed, as we will be communicating to the TMC4671 through SPI. STEP and DIR are for external motion controllers, like an FPGA steering the motors.

Lastly, we have a lot of PWM signals that we need to decide if we need or not. Let us go through the unneeded ones first: 

PWM_I is PWM input for steering the motor. We use SPI, and therefore do not need this connected. 
PWM_IDLE_H / PWM_IDLE_L are for setting idle states. We dont need any specific idle states, because our cube will be flat on the ground when not in use, which does not require a specific motor position/speed/torque etc. Therefore this will not be connected

Both of the above are PWM inputs in perspective of the TMC4671. The rest of the PWM signals are PWM outputs, which will be going to our driver gates. These are essential for 
motor operation, and is part of the core functionality of the controller. Therefore we route these outputs to the driver gates, which will in return drive the motors. 
